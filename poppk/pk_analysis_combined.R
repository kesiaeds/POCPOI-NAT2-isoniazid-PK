...
has_tidyvpc <- requireNamespace("tidyvpc", quietly = TRUE)
...
# nlmixr2 expects lowercase 'cens' for M3 BLQ handling. Map from CENS if present.
...
if ("cens" %in% names(dat)) {
  blq <- sum(dat$EVID == 0 & dat$cens == 1, na.rm = TRUE)
  cat("BLQ observations (CENS=1, handled by M3):", blq, "of",
      sum(dat$EVID == 0), "(", round(100*blq/sum(dat$EVID == 0), 1), "% )\n")
}
...
if (has_tidyvpc) {
  vpc_obj <- tidyvpc::vpcPlot(fit, n = 500, show = list(obs_dv = TRUE))
  ggsave("output/figures/vpc.pdf", vpc_obj, width = 7, height = 5)
} else {
  cat("\ntidyvpc not installed; skipping VPC. Re-run after installing it.\n")
}
